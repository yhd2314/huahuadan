...
# 加载数据并建立模型（这部分应该在应用启动时运行一次）
# 注意：这里假设stomach数据已经加载到环境中
dd <- datadist(stomach)
options(datadist = "dd")

# 建立Cox模型
cph <- cph(Surv(Survival.months, status_code1111) ~ Age + Grade + Nstage + Radiotherapy + RNE,
           data = stomach, x = TRUE, y = TRUE, surv = TRUE)

# 保存模型
saveRDS(cph, "stomach_model.rds")
